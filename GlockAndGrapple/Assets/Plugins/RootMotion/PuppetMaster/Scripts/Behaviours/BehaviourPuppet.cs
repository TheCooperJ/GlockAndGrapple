...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		public void Reset(Vector3 position, Quaternion rotation) {
			...
			foreach (Muscle m in puppetMaster.muscles) {
				m.state.pinWeightMlp = puppetMaster.state == PuppetMaster.State.Alive? 1f: 0f;
				m.state.muscleWeightMlp = puppetMaster.state == PuppetMaster.State.Alive? 1f: puppetMaster.stateSettings.deadMuscleWeight;
				m.state.muscleDamperAdd = puppetMaster.state == PuppetMaster.State.Alive? 0f: puppetMaster.stateSettings.deadMuscleDamper;
